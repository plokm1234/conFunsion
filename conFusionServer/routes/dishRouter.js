...
.post(cors.corsWithOptions, authenticate.verifyUser, authenticate.verifyAdmind, (req, res, next) => {
...
.put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    res.statusCode = 403;
    res.end('PUT operation not support on /dishes/' + req.params.dishId + '/comments');
})
.delete(cors.corsWithOptions, authenticate.verifyUser, authenticate.verifyAdmind, (req, res, next) => {
    Dishes.findById(req.params.dishId)
    .then((dish) => {
...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    res.statusCode = 403;
    res.end('POST operation not support on /' + req.params.dishId + '/comments/' + req.params.commentId);
})
.put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    Dishes.findById(req.params.dishId)
    .then((dish) => {
        const authorId = dish.comments.id(req.params.commentId).author._id;
        if (dish != null && dish.comments.id(req.params.commentId) != null && authorId.equals(req.user._id)) {
            if (req.body.rating) {
            ...
            err.status = 404;
            return next(err);
        } else if (!authorId.equals(req.user._id)) {
